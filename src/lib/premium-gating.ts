 ...
 *
 * IMPORTANT: Uses the native MongoDB driver (connectToDatabase) for user lookup
 * to ensure it reads from the same database as the authentication system.
 */
 ...
 *
 * Uses the native MongoDB driver to ensure it reads from the same database
 * as the authentication system (avoids Mongoose database mismatch issues).
 */
export async function checkPremiumStatus(userId: string): Promise<PremiumUser | null> {
  ...
  try {
    ...
    // Use native MongoDB driver — same as the auth system — to avoid DB mismatch
    ...
    try {
      ...
      logger.warn('PREMIUM_GATING', 'OBJECTID_PARSE_ERROR', 'OBJECTID_PARSE_ERROR', {
        requestId,
        userId,
    ...
    if (!user) {
      logger.warn('PREMIUM_GATING', 'USER_NOT_FOUND', 'USER_NOT_FOUND', {
        requestId,
        userId,
        dbName,
      });

      // Diagnostic: sample users in the collection
    ...
    // Map plan to tier — support both nested and flat schemas
    ...
    const userPlan: string =
      user.subscription?.plan || user.plan || 'basic';
    const tier: PremiumTier = planTierMap[userPlan] || 'basic';
    ...
    const premiumUser: PremiumUser = {
      ...
      tier,
      subscription: {
        status: (user.subscription?.status || user.subscriptionStatus || 'active') as
          | 'active'
          | 'inactive'
          | 'cancelled',
    ...
    logger.info('PREMIUM_GATING', 'CRM_ACCESS_GRANTED', 'CRM_ACCESS_GRANTED', {
      userId,
      tier,
      features: premiumUser.crmFeatures,
    });
    return premiumUser;
  } catch (error: any) {
    logger.error(
...
export function requirePremiumCRM(
  ...
  return async (request: NextRequest): Promise<NextResponse> => {
    ...
    try {
      logger.info('CRM_GATE', 'REQUEST_START', 'REQUEST_START', {
        requestId,
        url: request.url,
      });
      ...
      if (!authResult.success || !authResult.user) {
        logger.warn('CRM_GATE', 'AUTH_FAILED', 'AUTH_FAILED', {
          requestId,
      ...
      const userId =
        authResult.user.id || authResult.user._id?.toString();
      logger.debug('CRM_GATE', 'USER_AUTHENTICATED', 'USER_AUTHENTICATED', {
        requestId,
        userId,
      });
      ...
      if (!premiumUser) {
        logger.warn('CRM_GATE', 'USER_NOT_FOUND', 'USER_NOT_FOUND', {
          requestId,
          userId,
        });
        return NextResponse.json(
      ...
      logger.debug('CRM_GATE', 'ACCESS_GRANTED', 'ACCESS_GRANTED', {
        requestId,
        userId,
 ...
 */
export async function canAccessCRMFeature(
  userId: string,
  feature: keyof PremiumUser['crmFeatures']
): Promise<boolean> {
  try {
    const premiumUser = await checkPremiumStatus(userId);
    ...
    logger.error(
      ...
      'FEATURE_CHECK_ERROR',
      'FEATURE_CHECK_ERROR',
      { userId, feature, error: error.message },
      error
    );
    return false;
  }
