...
=======
Here are the high-quality image URLs you MUST use in the generated HTML for the hero section and features. You MUST use the ENTIRE URL string as the value for the \`src\` attribute in your \`<img>\` tags. DO NOT truncate or modify the URL in any way. The URLs are:
Hero Image URL: ${heroImages[0]?.url || 'NO_HERO_IMAGE'}
Feature Image 1 URL: ${featureImages[0]?.url || 'NO_FEATURE_IMAGE_1'}
Feature Image 2 URL: ${featureImages[1]?.url || 'NO_FEATURE_IMAGE_2'}
Testimonial Image URL: ${testimonialImages[0]?.url || 'NO_TESTIMONIAL_IMAGE'}
...
>>>>>>> 18adc7d (Fix: Implement robust post-processing to correct AI-truncated image URLs and resolve ERR_NAME_NOT_RESOLVED)
