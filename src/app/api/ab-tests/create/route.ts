...
export async function POST(request: NextRequest) {
  ...
  return runWithTraceContext(traceContext, async () => {
    try {
      ...
      if (!token) {
        logger.warn('ABTestingCreateAPI', 'POST', 'Authentication required', 'Authentication required', { traceId });
        return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
      }

      const decoded = verifyToken(token);
      if (!decoded) {
        logger.warn('ABTestingCreateAPI', 'POST', 'Invalid token', 'Invalid token', { traceId });
        return NextResponse.json({ error: 'Invalid token' }, { status: 401 });
      }

      const user = await getUserById(decoded.userId);
      if (!user) {
        logger.warn('ABTestingCreateAPI', 'POST', 'User not found', 'User not found', { userId: decoded.userId, traceId });
        return NextResponse.json({ error: 'User not found' }, { status: 404 });
      }

      // 2. Enterprise Plan Enforcement
      if (user.plan !== 'enterprise') {
        logger.warn('ABTestingCreateAPI', 'POST', 'Enterprise plan required', 'Enterprise plan required', { userId: user._id, plan: user.plan, traceId });
      ...
      if (!validationResult.success) {
        logger.warn('ABTestingCreateAPI', 'POST', 'Validation failed', 'Validation failed', { 
      ...
      logger.info('ABTestingCreateAPI', 'POST', 'Experiment created successfully', 'Experiment created successfully', {
        userId: user._id,
      ...
      logger.error('ABTestingCreateAPI', 'POST', 'Internal server error', 'Internal server error', {
