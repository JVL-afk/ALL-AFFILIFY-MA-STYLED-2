...
export async function GET(request: NextRequest) {
  ...
  return runWithTraceContext(traceContext, async () => {
    try {
      ...
      if (!token) {
        logger.warn('ABTestingListAPI', 'GET', 'Authentication required', 'Authentication required', { traceId });
        return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
      }

      const decoded = verifyToken(token);
      if (!decoded) {
        logger.warn('ABTestingListAPI', 'GET', 'Invalid token', 'Invalid token', { traceId });
        return NextResponse.json({ error: 'Invalid token' }, { status: 401 });
      }

      const user = await getUserById(decoded.userId);
      if (!user) {
        logger.warn('ABTestingListAPI', 'GET', 'User not found', 'User not found', { userId: decoded.userId, traceId });
        return NextResponse.json({ error: 'User not found' }, { status: 404 });
      }

      // 2. Enterprise Plan Enforcement
      if (user.plan !== 'enterprise') {
        logger.warn('ABTestingListAPI', 'GET', 'Enterprise plan required', 'Enterprise plan required', { userId: user._id, plan: user.plan, traceId });
      ...
      logger.info('ABTestingListAPI', 'GET', 'Experiments retrieved successfully', 'Experiments retrieved successfully', {
        userId: user._id,
      ...
      logger.error('ABTestingListAPI', 'GET', 'Internal server error', 'Internal server error', {
