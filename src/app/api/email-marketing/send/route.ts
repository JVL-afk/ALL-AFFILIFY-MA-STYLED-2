...
export async function POST(request: NextRequest) {
  ...
  return runWithTraceContext(traceContext, async () => {
    try {
      ...
      if (!authHeader) {
        logger.warn('EmailMarketingAPI', 'POST /send', 'Missing Authorization header', 'Missing Authorization header', {
      ...
      if (!payload) {
        logger.warn('EmailMarketingAPI', 'POST /send', 'JWT verification failed', 'JWT verification failed', {
          trace_id: traceContext.traceId,
          service: 'EmailMarketingAPI',
        });
      ...
      try {
        ...
        logger.warn('EmailMarketingAPI', 'POST /send', 'Permission denied', 'Permission denied', {
          trace_id: traceContext.traceId,
      ...
      if (!validationResult.success) {
        logger.warn('EmailMarketingAPI', 'POST /send', 'Invalid send campaign payload', 'Invalid send campaign payload', {
          trace_id: traceContext.traceId,
          user_id: payload.userId,
          service: 'EmailMarketingAPI',
      ...
      if (recipients.length > planLimits.maxRecipientsPerSend) {
        logger.warn('EmailMarketingAPI', 'POST /send', 'Recipient limit exceeded for plan', 'Recipient limit exceeded for plan', {
          trace_id: traceContext.traceId,
          user_id: payload.userId,
      ...
      if (!campaign) {
        logger.warn('EmailMarketingAPI', 'POST /send', 'Campaign not found or unauthorized', 'Campaign not found or unauthorized', {
          trace_id: traceContext.traceId,
          user_id: payload.userId,
      ...
      if (campaign.status === 'sent') {
        logger.warn('EmailMarketingAPI', 'POST /send', 'Cannot resend already sent campaign', 'Cannot resend already sent campaign', {
          trace_id: traceContext.traceId,
          user_id: payload.userId,
          campaign_id: campaignId,
      ...
      logger.info('EmailMarketingAPI', 'POST /send', 'Campaign sent successfully', 'Campaign sent successfully', {
        trace_id: traceContext.traceId,
        user_id: payload.userId,
        campaign_id: campaignId,
      ...
      logger.error('EmailMarketingAPI', 'POST /send', 'Failed to send campaign', 'Failed to send campaign', {
        trace_id: traceContext.traceId,
        service: 'EmailMarketingAPI',
        error: errorMessage,
      });
