...
export async function GET(request: NextRequest) {
  ...
  return runWithTraceContext(traceContext, async () => {
    try {
      ...
      if (!authHeader) {
        logger.warn('EmailMarketingAPI', 'GET /data', 'Missing Authorization header', 'Missing Authorization header', {
      ...
      if (!payload) {
        logger.warn('EmailMarketingAPI', 'GET /data', 'JWT verification failed', 'JWT verification failed', {
          trace_id: traceContext.traceId,
          service: 'EmailMarketingAPI',
        });
        return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
      ...
      try {
        ...
        if (!hasPlanOrHigher(payload, 'pro')) {
          logger.warn('EmailMarketingAPI', 'GET /data', 'Insufficient plan for analytics access', 'Insufficient plan for analytics access', {
            trace_id: traceContext.traceId,
        ...
        logger.warn('EmailMarketingAPI', 'GET /data', 'Permission denied', 'Permission denied', {
          trace_id: traceContext.traceId,
          user_id: payload.userId,
          service: 'EmailMarketingAPI',
        });
      ...
      try {
        ...
        logger.warn('EmailMarketingAPI', 'GET /data', 'Failed to fetch campaigns', 'Failed to fetch campaigns', {
          trace_id: traceContext.traceId,
      ...
      try {
        ...
        logger.warn('EmailMarketingAPI', 'GET /data', 'Failed to calculate stats', 'Failed to calculate stats', {
          trace_id: traceContext.traceId,
          user_id: payload.userId.toString(),
          service: 'EmailMarketingAPI',
          error: error instanceof Error ? error.message : String(error),
      ...
      try {
        ...
        logger.warn('EmailMarketingAPI', 'GET /data', 'Failed to fetch subscriber count', 'Failed to fetch subscriber count', {
          trace_id: traceContext.traceId,
          user_id: payload.userId.toString(),
          service: 'EmailMarketingAPI',
          error: error instanceof Error ? error.message : String(error),
      ...
      try {
        ...
        logger.warn('EmailMarketingAPI', 'GET /data', 'Failed to fetch templates', 'Failed to fetch templates', {
          trace_id: traceContext.traceId,
          user_id: payload.userId.toString(),
          service: 'EmailMarketingAPI',
          error: error instanceof Error ? error.message : String(error),
      ...
      try {
        ...
        logger.warn('EmailMarketingAPI', 'GET /data', 'Failed to fetch subscribers', 'Failed to fetch subscribers', {
          trace_id: traceContext.traceId,
          user_id: payload.userId.toString(),
          service: 'EmailMarketingAPI',
          error: error instanceof Error ? error.message : String(error),
      ...
      logger.info('EmailMarketingAPI', 'GET /data', 'Email marketing data retrieved successfully', 'Email marketing data retrieved successfully', {
        trace_id: traceContext.traceId,
        user_id: payload.userId,
        service: 'EmailMarketingAPI',
      ...
      logger.error('EmailMarketingAPI', 'GET /data', 'Error fetching email marketing data', 'Error fetching email marketing data', {
        trace_id: traceContext.traceId,
        service: 'EmailMarketingAPI',
        error: errorMessage,
      });
